...
Codex is the verified adapter. It runs `codex exec` with web search enabled, `--ephemeral`, `--sandbox read-only`, `--ignore-user-config`, `--ignore-rules`, and `--skip-git-repo-check`. Unsupported agents fail closed; Scope does not guess at an unsafe command.
...
- Escape invalidates the active generation and terminates only the current Scope helper and its dedicated Codex process group. Scope never uses `pkill` or `killall` for Codex.

## Escalation

**Open Agent** is the explicit escalation boundary. Only after the user selects it does Scope open normal interactive Codex, passing the selected image and concise Scope Search context. It is never opened when a search completes.
